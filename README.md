...
_Note:_ There are two scripts in the repository [\window-file-server\preconfigs] which can configure requirements on OpenStacks
- [setupWindows.bat] -- this scipt does not work when last tested, would recommned to access console to ensure the requirements
- [setupUbuntu.sh] -- this script should work, would recommend to verify or to run this script manually on SSH
...
2. Clone the repository.
  [git clone https://github.com/kachowtime/ansible-vuln-deployment.git]

3. Configure inventory -- edit [inventory.ini] with your target IP and users credentials:
...
- Verify that Ansible can communicate with the Windows file server over WinRM using [ansible -i inventory.ini windows_fileserver -m win_ping]
